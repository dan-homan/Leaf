...
**Evaluation gauge anchoring** (PSQT slot-mean + PAWN piece_val pin) — without this, training is non-identifiable because PSQT and `piece_val` redundantly encode material level, and the multi-writer merge protocol can amplify any drift geometrically:
- `TDLEAF_PIN_PAWN_VALUE = true` freezes `piece_val[PAWN]` at its init value, fixing the absolute material scale that `TDLEAF_K` would otherwise leave free; N/B/R/Q `piece_val` still adapt.
- Per-batch **gradient mean-centering** (`nnue_mean_center_psqt_gradients`) zeros the per-slot aggregate PSQT gradient (summed across all 8 buckets).
- Per-batch **post-Adam dw centering** in `nnue_apply_gradients` Pass 2 zeros the per-slot aggregate applied step — necessary because Adam's per-weight `1/√v_hat` breaks the zero-sum property of mean-centered gradients.
- Per-(slot, bucket) **init slot-means** are persisted in `.tdleaf.bin` v11 (88 floats); `nnue_recenter_psqt_slot_means()` snaps the per-slot aggregate back to the persisted target at every load AND every save (after the multi-writer merge, before the write).  Per-bucket distribution within a slot is left free, which is what lets the bucketed PSQT learn phase-dependent piece values.
- `--init-nnue-noprior` initialises PSQT to **uniform 100 cp** for all piece types (own=+V, enemy=−V), not zero — anchors `value[PAWN..QUEEN]` at 100 cp from move 1 so the PAWN pin lands on a sane value and SEE/material-accounting still works.

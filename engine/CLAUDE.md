...
**Key hyperparameters:** `TDLEAF_K = 240 cp` (sigmoid temperature), `TDLEAF_LAMBDA = 0.98` (single eligibility trace decay — decisive and draw games use the same value).  Five separate Adam LRs: FC (0.01), FT (1.0), FT bias (0.01), PSQT (10.0), piece_val (100.0).  Gradient clipping (L2 norm, 1.0) and AdamW weight decay (1e-4, FC+FT weights only).

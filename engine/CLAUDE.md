...
| `--psqt-opponent same\|clean\|anti` | Two-sided hypotheses (default `same` = one table for both sides).  `clean`: side A +ε vs the unperturbed net; `anti`: +ε vs −ε.  The self-play driver selects each side's PSQT table and its own TT/score hash per move, and plays each opening twice with colours swapped (PGN names `hypA`/`hypB`) |

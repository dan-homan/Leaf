...
| `--psqt-noise <frac>` | Actor-only **parameter-space** perturbation: the positional part of each (piece type, PSQT bucket) group of PSQT entries is scaled by (1+ε), ε ~ N(0, frac) clamped to [−1, 3·frac]; material (the group's usage-weighted mean) is untouched.  Applied after the state load; a TDLEAF build refuses it unless `TDLEAF_FREEZE=1`, since a learner would save the perturbed weights.  Needs `--psqt-noise-ref`.  Labels stay clean with no special code — the statics include it and the learner's `--refresh-scores` removes it |
| `--psqt-opponent same\|clean\|anti` | Two-sided hypotheses (default `same` = one table for both sides).  `clean`: side A +ε vs the unperturbed net; `anti`: +ε vs −ε.  The self-play driver selects each side's PSQT table and its own TT/score hash per move, side A alternating colour game by game (PGN names `hypA`/`hypB`).  One opening per game by default, so a leg keeps the unperturbed opening sequence of its `--seed`; `--pair-openings` plays each twice with colours swapped, for the measurement arms' pentanomial Elo |
| `--psqt-noise-seed <n>` / `--psqt-noise-ref <file>` | Draw seed (`selfplay_run.py` shares one per generation across actors) / usage-weighted material reference from `scripts/psqt_decomp.py --write-ref` |

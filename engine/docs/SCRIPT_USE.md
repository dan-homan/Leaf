...
| `--corpus-window N`                          | `4`                                                    | Dilute this run's dump with the archived `corpus.tsv.gz` of up to N prior iterations from the `--continue` chain, holding the **total** row count fixed (`--corpus-rows`) so epoch cost is unchanged. Worth ~45 Elo — row-matched arms differing only in game diversity scored +112.9 (500k games) vs +148.7 (2.5M games) vs the classical anchor (`Offline_Learning_Investigation.md` Part 2). `0` disables it (pre-A1 behaviour). Needs `--continue` |

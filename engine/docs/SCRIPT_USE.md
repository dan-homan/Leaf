...
| `--no-repeat`                                | off                                                    | Play each opening once during generation (forwards `match.py --no-repeat`). Essential for frozen pairs (`TDLEAF_FREEZE=1`): identical deterministic engines replay the same game from an opening every time — including the color-swapped repeat — so cap `--games` at the opening count |
| `--dedup-corpus`                             | off; auto-on when `TDLEAF_FREEZE` is set               | Drop duplicate corpus rows (identical in every field except gid) during corpus assembly. Duplicate games straddle the trainer's by-game train/val split (different gids), so training on them both overfits and leaks validation |

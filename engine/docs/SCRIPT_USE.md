...
| `--bt-rows R`                                | `both`                                                 | Which corpus rows to train on: `leaf` (static-eval rows, depth 0), `root` (search-score rows, depth > 0), or `both`. Filtered at trainer load, so the archived `corpus.tsv.gz` stays complete and row-mode sweeps can rerun from one corpus via `--skip-online --corpus` |
...
| `--online-lr-comp F`                         | off                                                    | Build the actor/learner binary with `TDLEAF_RBAR_LR_COMP=F`, scaling the **online** FT-weight and PSQT LRs by `F` to offset the Adam step-size rise `TDLEAF_FEATURE_RBAR` produces for free (docs 6.12; `0.68` matches the measured 1.47×).  The batch trainer is built without it.  Requires `TDLEAF_FEATURE_RBAR` in the environment; forces a recompile of that binary. |

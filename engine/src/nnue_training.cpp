...
#if WDL_HEAD
// Auxiliary WDL head — FP32 shadow weights.  Phase 1a: session-local Adam,
// absolute-value persistence, no counts / delta-merge (see docs/WDL_HEAD.md).
...
#if WDL_HEAD
// WDL head Adam moments — session-local (zeroed each start), not persisted.
// A single shared step counter is correct because the head is dense: every
// weight receives a gradient on every apply that had any WDL signal.
...
static uint32_t t_wdl_session = 0;   // session-local Adam step counter (bias correction)
#endif
...
void nnue_init_zero_weights(int prior_mode)
{
...
#if WDL_HEAD
    // Separate RNG stream for the WDL head so the scalar FC weights draw the same
    // sequence as a non-WDL build (fresh-init scalar net stays bit-identical).
    std::mt19937 wdl_rng(1337);
    std::normal_distribution<float> wdl_nd(0.0f, 0.02f);
#endif
    ...
    for (int s = 0; s < NNUE_LAYER_STACKS; s++) {
        ...
        // 100 cp/pawn PSQT material signal and preventing material-aware play from game 1.
        ...
        for (int o = 0; o < NNUE_WDL_OUT; o++) {
            for (int i = 0; i < NNUE_WDL_IN; i++)
                wdl_weights_f32[s][o][i] = wdl_nd(wdl_rng);
    ...
    //
    // Under pure-PSQT the bucketed PSQT is the ONLY material channel; there is no
    // dense piece_val to invert, so material is as robust as the sparse-but-many
    // PSQT feature rows above.
    //
    ...
    //
    // noprior: PSQT = uniform 100 cp per piece type (P=N=B=R=Q=100).
    ...
    if (ft_weights_f32) {
        ...
        // material-accounting semantics that consume value[PAWN] as cp.
        ...
        // knights penalise edges).  We pre-compute the per-(slot, bucket) PST
        // mean and subtract it, so each piece type's init material level is
        // exactly MATERIAL_CP[slot] and the PST contributes zero-mean positional
        // deviation only.  (Retained for init reproducibility with the pure-PSQT
        // reference nets; the old slot-mean recenter that consumed this is gone.)
...
#if WDL_HEAD
    // The WDL head has no int8 source in the .nnue; fresh-init it here.  A v13
    // .tdleaf.bin load overwrites this; a v12/older or absent file leaves it
    // at this init.  Separate RNG stream leaves scalar copies untouched.
    std::mt19937 wdl_rng(1337);
    std::normal_distribution<float> wdl_nd(0.0f, 0.02f);
#endif
    for (int s = 0; s < NNUE_LAYER_STACKS; s++) {
...
#if WDL_HEAD
    // 8. Auxiliary WDL head (read-out only in Phase 1a — does not affect the
    //    scalar score above).  Input = fc2_in[0..31] plus the STM-POV cp eval
    //    (material channel, scaled).  Output = softmax over {win, draw, loss}.
    {
        ...
        for (int o = 0; o < NNUE_WDL_OUT; o++) {
            ...
            sum += w[NNUE_L2_PADDED] * act.wdl_mat;
...
// A faithful copy of the fc2_in -> FT portion of nnue_accumulate_gradients, but
// deliberately EXCLUDING three scalar-only channels:
//   - FC2 output-layer weights (l2_*): the scalar head, not shared with WDL.
//   - the FC0 passthrough (fc0_raw[15]): a direct score channel that fc2_in has
//     no dependence on, so its WDL gradient is identically zero.
//   - PSQT: the WDL head's only PSQT dependence is via the wdl_mat input, which
//     is treated as stop-gradient (protects the pure-PSQT material scale).
                                    ...
                                    NNUEGradBuf *gb)
{
    ...
    // FC0 inputs -> accumulator -> FT (SqrCReLU inverse)
    float g_l0_in[NNUE_L0_INPUT] = {};
    for (int o = 0; o < NNUE_L0_SIZE; o++) {
        if (g_fc0_raw[o] == 0.0f) continue;
        const float *w = &l0_weights_f32[s][o * NNUE_L0_INPUT];
    ...
    // FT weight gradient + dirty-row marking (NO PSQT).
    for (int p = 0; p < 2; p++) {
        int persp  = (p == 0) ? stm_p : (stm_p ^ 1);
        float *g_a = g_acc[persp];
        for (int k = 0; k < (int)act.n_ft[persp]; k++) {
...
// ---------------------------------------------------------------------------
// nnue_accumulate_wdl_gradients — backprop the auxiliary WDL head for one ply.
// d_logits = softmax(logits) − target (STM POV), the softmax-CE gradient; scale
// folds the loss weight and id-stability weight.  Updates the head's own weights
// always.  With WDL_TRUNK_GRAD, the gradient w.r.t. fc2_in is ALSO backpropped
// into the shared trunk (FC1/FC0/FT) as an auxiliary task — but the wdl_mat
// (material) channel stays stop-gradient, so PSQT / the eval scale are untouched.
// Without WDL_TRUNK_GRAD (default), the head is a pure read-out.
// ---------------------------------------------------------------------------
                                   ...
                                   float scale, NNUEGradBuf *gb)
{
    ...
    int s = act.stack;
#if WDL_TRUNK_GRAD
    float g_fc2_in[NNUE_L2_PADDED] = {};   // dLoss/dfc2_in accumulated over outputs
#endif
    for (int o = 0; o < NNUE_WDL_OUT; o++) {
        ...
        gw[NNUE_L2_PADDED] += g * act.wdl_mat;   // head weight on wdl_mat; input is stop-grad
        gb->grad_wdl_b[s][o] += g;
    }
#if WDL_TRUNK_GRAD
...
#if WDL_HEAD
    const float wdl_lr     = lr_scale * warmup_factor * TDLEAF_ADAM_WDL_LR0;  // session-local head
#endif
...
#if WDL_HEAD
        // Auxiliary WDL head — session-local Adam (shared t_wdl_session counter),
        // no weight decay, no int8 clamp (float head), no delta/count tracking.
        for (int o = 0; o < NNUE_WDL_OUT; o++) {
            for (int i = 0; i < NNUE_WDL_IN; i++) {
...
#if WDL_HEAD
    // Auxiliary WDL head — session-local Adam (shared t_wdl_session), float weights.
    for (int o = 0; o < NNUE_WDL_OUT; o++) {
        for (int i = 0; i < NNUE_WDL_IN; i++)
            if (grad_wdl_w[s][o][i] != 0.0f) {
...
//
// Version 11 (legacy): added persisted PSQT init slot-means (11×PSQT_BKTS
//   floats) for the old gauge-anchoring recenter step.
//
// Version 12 additions (current): pure-PSQT — the dense piece_val channel and
//   the gauge machinery were removed.  The DROPPED sections are:
//     - the piece_val weight section (float32[6] + uint32[6], v5+)
//     - piece_val v inside the Adam v section (float32[6], v6+)
//     - piece_val m inside the Adam m section (float32[6], v7+)
//     - the v11 PSQT init slot-means block (11×PSQT_BKTS floats)
...
#if WDL_HEAD
static const uint32_t TDLEAF_VERSION = 13u;  // v13 adds the trailing WDL head section
#else
...
bool nnue_save_fc_weights(const char *path)
{
    ...
    if (cur) {
        ...
        bool ok = (fread(&magic, 4, 1, cur) == 1 &&
                   ...
                   magic == TDLEAF_MAGIC &&
                   (version == TDLEAF_VERSION || version == 13u || version == 12u || version == 11u || version == 10u || version == 9u || version == 8u || version == 7u || version == 6u || version == 5u || version == 4u || version == 3u || version == 2u));
        if (ok && version >= 10u) {
    ...
    // Adam m section (v7): FC m + FT bias m + sparse PSQT m.
...
#if WDL_HEAD
    // WDL head section (v13): absolute FP32 weights + biases, no counts/deltas.
    // Written raw (no TDLEAF_SCALE — head weights are already O(0.01–1)).  A v12
    // reader ignores these trailing bytes; a v13 WDL reader restores them.
    for (int s = 0; s < NNUE_LAYER_STACKS; s++) {
        for (int o = 0; o < NNUE_WDL_OUT; o++)
...
bool nnue_load_fc_weights(const char *path)
{
    ...
    if (version != 2u && version != 3u && version != 4u && version != 5u && version != 6u && version != 7u && version != 8u && version != 9u && version != 10u && version != 11u && version != 12u && version != 13u && version != TDLEAF_VERSION) {
    ...
    // Dense piece value section (v5-v11 only; removed in v12).
    // Read and discard to stay byte-aligned when loading an older file.
    if (ok && version >= 5u && version <= 11u) {
        if (version >= 9u) {
            float tmp_pv[6]; uint32_t tmp_pvc[6];
            if (fread(tmp_pv,  sizeof(float),    6, f) != 6 ||
                fread(tmp_pvc, sizeof(uint32_t), 6, f) != 6) ok = false;
        } else {
            float tmp_pv[6][NNUE_PSQT_BKTS]; uint32_t tmp_pvc[6][NNUE_PSQT_BKTS];
            if (fread(tmp_pv,  sizeof(float),    6*NNUE_PSQT_BKTS, f) != (size_t)(6*NNUE_PSQT_BKTS) ||
                fread(tmp_pvc, sizeof(uint32_t), 6*NNUE_PSQT_BKTS, f) != (size_t)(6*NNUE_PSQT_BKTS)) ok = false;
        }
    }
    ...
    // v11 PSQT init slot-means block (removed in v12) — read and discard.
    if (ok && version == 11u) {
...
#if WDL_HEAD
    // WDL head section (v13+): restore absolute head weights.  Older files leave
    // the head at its fresh init (nnue_init_fp32_weights).  Non-fatal if truncated.
    if (ok && version >= 13u) {
        bool wok = true;
        for (int s = 0; s < NNUE_LAYER_STACKS && wok; s++) {
            ...
            if (wok)
                wok = (fread(wdl_bias_f32[s], sizeof(float), NNUE_WDL_OUT, f) == (size_t)NNUE_WDL_OUT);
        }
        // Truncated head → keep the fresh-init values already in place.
    }
#endif

...
- `v_ft_w` (FT per-weight second moment, ~92 MB) is now sparsely persisted in .tdleaf.bin v8 for all dirty feature rows (same set as FT weight rows).  This eliminates the per-restart FT v cold-start that caused a 10–20 Elo dip for the first ~1000 Adam steps.  A per-session FT LR warmup (`TDLEAF_FT_SESSION_WARMUP=100` Adam steps) damps FT updates while v accumulates, covering both loaded (warmed) and fresh rows.
